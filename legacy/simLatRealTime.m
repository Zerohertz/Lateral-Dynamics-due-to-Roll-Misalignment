function [Sol, tit] = simLatRealTime(endAngle, delAngle)
W = 0.258;
T = 2.7*9.81; %
...
for theL = Ang
    ...
    elseif the_M < theL && theL < the_ML
        stats = 2
        tit(i) = "Circumferential Slippage";
        save("status.mat", "stats");
    elseif the_ML < theL
        stats = 3
        tit(i) = "Lateral Slippage";
        save("status.mat", "stats");
    else
        stats = 4
        save("status.mat", "stats");
    ...
    Sol{i} = bvp5c(@(x,y,r) bvpfcn(x,y,r), @bcfcn, solinit);
